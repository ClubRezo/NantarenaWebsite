$(function(){
	...
	var tournamentProgress = document.getElementById('tournamentProgress');
	var tournamentCountdown = document.getElementById('tournamentCountdown');
	// Création de l'objet 'date' (année / mois / jour)
	// Attention les mois commencent à 0 !
	var ts = new Date(2014, 09, 1);
	
	if((new Date()) > ts){
		// L'évènement est passé, on affiche les inscriptions et on cache le compteur
		tournamentCountdown.style.display = 'none';
	}
	else {
